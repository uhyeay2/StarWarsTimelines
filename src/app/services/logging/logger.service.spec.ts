...
import { LoggerService, LOG_CONFIG, LogConfig, LogEntry, LogSink, LogLevel } from './logger.service';
...
/**
 * Extracts the serialized context from a colorized console call.
 *
 * Colorized layout: [fmt, prefixStyle, messageStyle, serialized?]
 * Non-colorized layout: [prefix, message, serialized?]
 */
function getSerialized(call: unknown[], colorize: boolean): unknown {
  return colorize ? call[3] : call[2];
}

/**
 * Extracts the message string from a console call.
 * Colorized: message is embedded in the format string as %c<message>.
 * Non-colorized: message is the second argument.
 */
function getMessage(call: unknown[], colorize: boolean): string {
  if (colorize) {
    const fmt = call[0] as string;
    const match = fmt.match(/%c([^\s]+)\s+\[/);
    return match ? match[1] : fmt;
  }
