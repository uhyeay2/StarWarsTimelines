...
import {
  ...
  standaloneUnitIdOf,
  parsedUnitId,
  editSourceSelectionKey,
  editSourceSelectionKeys,
} from './source-option-keys';
export { editSourceSelectionKey, editSourceSelectionKeys };
...
function unitNode(
  ...
  const label = isContainerOrCollectionUnit(unit.unitType) ? containerLabel(unit) : leafLabel(unit);
  if (kids.length === 0) {
    return { value, label };
  }
  return {
