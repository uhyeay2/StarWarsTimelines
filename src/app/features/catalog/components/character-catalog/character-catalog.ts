...
import { formatGalacticYearRange } from '../../../../shared/utils/galactic-year';
...
export class CharacterCatalog implements OnInit {
  ...
  submitAdd(): void {
    ...
    runOperation({
      ...
      idleValue: false,
      error: this.addError,
      operation: this.characterService.createCharacter(input),
      onSuccess: (item) => {
        if (item) {
          this.resetAddForm();
          this.addOpen.set(false);
  ...
  saveEdit(): void {
    ...
    runOperation({
      ...
      operation: this.characterService.updateCharacter(id, input),
      onSuccess: (updated) => {
        if (updated) {
          this.cancelEdit();
        }
  ...
  confirmDelete(): void {
    ...
    runOperation({
      ...
      idleValue: null,
      error: this.actionError,
      operation: this.characterService.deleteCharacter(id),
